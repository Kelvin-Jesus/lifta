...
export const WorkoutDeck: Component<WorkoutDeckProps> = (props) => {
  ...
  return (
    <div
      class="w-full h-[100dvh] max-h-[100dvh] flex flex-col bg-neutral-950 text-neutral-100 overflow-hidden select-none"
      data-testid="workout-deck"
    >
      {/* Top Header Bar */}
      <header class="p-3 border-b border-neutral-850 bg-neutral-900/40 backdrop-blur-md flex-shrink-0">
        <div class="flex items-center justify-between mb-2">
          <div class="flex items-center gap-2">
            <button
              type="button"
              onClick={props.onExit}
              class="w-8 h-8 rounded-full bg-neutral-800 hover:bg-neutral-700 flex items-center justify-center text-neutral-400 hover:text-white"
              aria-label="Sair"
              data-testid="btn-deck-exit"
            >
              ✕
            </button>
            <h1 class="text-sm font-bold tracking-tight text-neutral-100 truncate max-w-[200px]">
              {session()?.routineName ?? 'Treino Ativo'}
            </h1>
          </div>

          <div class="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-neutral-900 border border-neutral-800">
            <span class="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span
              class="text-xs font-mono font-bold text-neutral-200 tabular-nums"
        ...
        <div class="mt-2 py-1 px-2.5 rounded-lg bg-blue-950/25 border border-blue-900/30 flex items-center justify-between text-[11px]">
          ...
          <span class="font-mono text-blue-200 font-semibold text-[10px]">
            +2.5 kg ou +1 rep vs última sessão
          </span>
        </div>
      </header>

      {/* Horizontal Carousel (100dvh snap-mandatory deck) */}
      <main
        ref={carouselRef}
        onScroll={handleScroll}
        class="flex-1 w-full overflow-x-auto snap-x snap-mandatory flex scroll-smooth no-scrollbar"
      ...
      >
        <For each={exercises()}>
          {(exercise, idx) => (
            <div
              class="w-full min-w-full snap-center px-4 py-4 overflow-y-auto flex flex-col items-center"
      ...
      <Show when={toastMessage()}>
        <div
          class="fixed top-16 left-1/2 -translate-x-1/2 z-50 px-4 py-2 rounded-full bg-neutral-900 border border-neutral-700 text-xs text-neutral-200 shadow-xl shadow-black animate-in fade-in slide-in-from-top-2 duration-200 select-none"
